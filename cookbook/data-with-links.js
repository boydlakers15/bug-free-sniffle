const data = {
	data: [
		{
			id: 1,
			attributes: {
				title: 'Pancakes',
				time: '20 min',
				level: 'easy',
				createdAt: '2023-03-28T17:52:01.194Z',
				updatedAt: '2023-03-28T21:50:24.117Z',
				publishedAt: '2023-03-28T21:24:04.418Z',
				ingredients: {
					0: '1 ½ cups all-purpose flour',
					1: '3 ½ teaspoons baking powder',
					2: '1 tablespoon white sugar',
					3: '¼ teaspoon salt, or more to taste ',
					4: '1 ¼ cups milk',
					5: '3 tablespoons butter, melted',
					6: '1 egg'
				},
				instructions: {
					1: 'Sift flour, baking powder, sugar, and salt together in a large bowl. Make a well in the center and add milk, melted butter, and egg; mix until smooth.',
					2: 'Heat a lightly oiled griddle or pan over medium-high heat. Pour or scoop the batter onto the griddle, using approximately 1/4 cup for each pancake; cook until bubbles form and the edges are dry, about 2 to 3 minutes. Flip and cook until browned on the other side. Repeat with remaining batter. '
				},
				img: {
					data: {
						id: 1,
						attributes: {
							name: 'Pancakes.webp',
							alternativeText:
								'https://www.allrecipes.com/thmb/WLHsZm_B4rdxpmAMFrIbmCSYW08=/282x188/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/21014-Good-old-Fashioned-Pancakes-mfs_002-0e249c95678f446291ebc9408ae64c05.jpg',
							caption: null,
							width: 1500,
							height: 1000,
							formats: {
								thumbnail: {
									name: 'thumbnail_Pancakes.webp',
									hash: 'thumbnail_Pancakes_6ffed0652b',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 234,
									height: 156,
									size: 7.23,
									url: '/uploads/thumbnail_Pancakes_6ffed0652b.webp'
								},
								medium: {
									name: 'medium_Pancakes.webp',
									hash: 'medium_Pancakes_6ffed0652b',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 750,
									height: 500,
									size: 46.05,
									url: '/uploads/medium_Pancakes_6ffed0652b.webp'
								},
								small: {
									name: 'small_Pancakes.webp',
									hash: 'small_Pancakes_6ffed0652b',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 500,
									height: 333,
									size: 23.32,
									url: '/uploads/small_Pancakes_6ffed0652b.webp'
								},
								large: {
									name: 'large_Pancakes.webp',
									hash: 'large_Pancakes_6ffed0652b',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 1000,
									height: 667,
									size: 66.11,
									url: '/uploads/large_Pancakes_6ffed0652b.webp'
								}
							},
							hash: 'Pancakes_6ffed0652b',
							ext: '.webp',
							mime: 'image/webp',
							size: 138.17,
							url: '/uploads/Pancakes_6ffed0652b.webp',
							previewUrl: null,
							provider: 'local',
							provider_metadata: null,
							createdAt: '2023-03-28T17:50:27.333Z',
							updatedAt: '2023-03-29T15:25:12.063Z'
						}
					}
				}
			}
		},
		{
			id: 2,
			attributes: {
				title: 'Tiramisu',
				time: '35 min ',
				level: 'medium',
				createdAt: '2023-03-28T18:42:55.393Z',
				updatedAt: '2023-03-28T21:25:02.382Z',
				publishedAt: '2023-03-28T21:25:02.378Z',
				ingredients: {
					0: '6 large egg yolks',
					1: '¾ cup white sugar',
					2: '⅔ cup milk',
					3: '1 ¼ cups heavy cream',
					4: '½ teaspoon vanilla extract',
					5: '1 pound mascarpone cheese',
					6: '¼ cup strong brewed coffee',
					7: '2 tablespoons rum',
					8: '2 packages ladyfinger cookies',
					9: '1 tablespoon unsweetened cocoa powder'
				},
				instructions: {
					1: 'Make the Filling: Cook the egg yolks, sugar, and milk until slightly thickened. Let cool slightly, then chill in the fridge for about an hour. When the filling has fully chilled, mix in mascarpone cheese.',
					2: 'Make the Whipped Cream: Beat heavy cream with vanilla extract until stiff peaks form.',
					3: 'Soak the Ladyfingers: Combine coffee and rum in a small bowl. Pour mixture over ladyfingers that have been split in half lengthwise.',
					4: 'Assemble the Tiramisu: Line the bottom of a baking dish with soaked ladyfingers. Spread half of the mascarpone mixture over the ladyfingers, then half of the whipped cream over that. Repeat in the same order. Dust with cocoa powder.'
				},
				img: {
					data: {
						id: 2,
						attributes: {
							name: 'tiramisu.webp',
							alternativeText:
								'https://www.allrecipes.com/thmb/xy_G3p4XDkuKDG4LDAivUh-hWLI=/282x188/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/tiramisu-ii-ca809422e2244ba0a023b07b3112d4fd.jpeg',
							caption: null,
							width: 750,
							height: 750,
							formats: {
								thumbnail: {
									name: 'thumbnail_tiramisu.webp',
									hash: 'thumbnail_tiramisu_7678594607',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 156,
									height: 156,
									size: 4.76,
									url: '/uploads/thumbnail_tiramisu_7678594607.webp'
								},
								small: {
									name: 'small_tiramisu.webp',
									hash: 'small_tiramisu_7678594607',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 500,
									height: 500,
									size: 24.57,
									url: '/uploads/small_tiramisu_7678594607.webp'
								}
							},
							hash: 'tiramisu_7678594607',
							ext: '.webp',
							mime: 'image/webp',
							size: 45.4,
							url: '/uploads/tiramisu_7678594607.webp',
							previewUrl: null,
							provider: 'local',
							provider_metadata: null,
							createdAt: '2023-03-28T18:26:43.145Z',
							updatedAt: '2023-03-29T15:24:38.683Z'
						}
					}
				}
			}
		},
		{
			id: 3,
			attributes: {
				title: 'Beef Empanadas',
				time: '2 hrs 25 mins',
				level: 'advanced',
				createdAt: '2023-03-28T19:28:38.968Z',
				updatedAt: '2023-03-29T15:36:57.114Z',
				publishedAt: '2023-03-29T15:36:57.111Z',
				ingredients: {
					1: '1 tablespoon extra-virgin olive oil',
					2: '1 pound yellow onions, chopped',
					3: '6 scallions, chopped',
					4: '1 green bell pepper, chopped',
					5: '2 pounds ground beef',
					6: '2 tablespoons dried oregano',
					7: '1 ½ tablespoons salt',
					8: '1 tablespoon cayenne pepper',
					9: '1 egg white, beaten',
					10: '1 ½ teaspoons ground cumin'
				},
				instructions: {
					1: 'Heat oil in a large Dutch oven over medium heat. Add onions, scallions, and bell pepper. Cover and cook, stirring occasionally, until onions are translucent, 10 to 15 minutes. ',
					2: 'Add ground beef, oregano, salt, cayenne pepper, and cumin to the pot. Cook and stir beef until no longer pink, 8 to 10 minutes. Remove from the heat and allow to cool slightly, then cover and place in the refrigerator until meat is completely cooled, about 1 hour. ',
					3: 'Remove filling from the refrigerator and transfer to a colander to drain. Return filling to the Dutch oven. ',
					4: 'Preheat the oven to 400 degrees F (200 degrees C). Liberally grease a baking sheet with butter. ',
					5: 'Place about 2 tablespoons filling into the center of empanada disk. Fold empanada in half into a crescent shape, press the edges of dough together with a fork or your fingers to seal, and place onto the prepared baking sheet. Repeat with remaining empanadas and filling. Brush tops with egg white. ',
					6: 'Bake in the preheated oven until golden brown, about 20 minutes.'
				},
				img: {
					data: {
						id: 3,
						attributes: {
							name: 'empanadas.webp',
							alternativeText:
								'https://www.allrecipes.com/thmb/-tq1tUXxAEPYsWthU_du_EjpM54=/282x188/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/8792318-011ed4c9eb804a3c9fb46ec5cb59397d.jpg',
							caption: null,
							width: 750,
							height: 500,
							formats: {
								thumbnail: {
									name: 'thumbnail_empanadas.webp',
									hash: 'thumbnail_empanadas_2d715e32ff',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 234,
									height: 156,
									size: 4.98,
									url: '/uploads/thumbnail_empanadas_2d715e32ff.webp'
								},
								small: {
									name: 'small_empanadas.webp',
									hash: 'small_empanadas_2d715e32ff',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 500,
									height: 333,
									size: 12.01,
									url: '/uploads/small_empanadas_2d715e32ff.webp'
								}
							},
							hash: 'empanadas_2d715e32ff',
							ext: '.webp',
							mime: 'image/webp',
							size: 20.07,
							url: '/uploads/empanadas_2d715e32ff.webp',
							previewUrl: null,
							provider: 'local',
							provider_metadata: null,
							createdAt: '2023-03-28T19:28:32.293Z',
							updatedAt: '2023-03-29T15:23:40.021Z'
						}
					}
				}
			}
		},
		{
			id: 4,
			attributes: {
				title: 'Chickpea Curry',
				time: '30 mins',
				level: 'medium',
				createdAt: '2023-03-28T19:59:51.928Z',
				updatedAt: '2023-03-28T21:51:32.948Z',
				publishedAt: '2023-03-28T21:23:24.081Z',
				ingredients: {
					1: '1 onion, chopped',
					2: '2 cloves garlic, minced',
					3: '2 teaspoons minced fresh ginger root',
					4: '1 (15 ounce) can chickpeas, drained',
					5: '1 (14.5 ounce) can diced tomatoe',
					6: '1 (14 ounce) can coconut milk',
					7: '1 sweet potato, cubed',
					8: '1 tablespoon garam masala',
					9: '1 teaspoon ground cumin',
					10: '1 teaspoon ground turmeric',
					11: '¼ teaspoon red chile flakes',
					12: '½ teaspoon salt',
					13: '1 cup baby spinach'
				},
				instructions: {
					1: 'Heat oil in a skillet over medium heat. Cook onion, garlic, and ginger in hot oil until softened, about 5 minutes. Add chickpeas, tomatoes, coconut milk, and sweet potato. Bring to a boil, reduce heat to low, and simmer until tender, about 15 minutes. ',
					2: 'Season with garam masala, cumin, turmeric, salt, and chile flakes. Add spinach right before serving. '
				},
				img: {
					data: {
						id: 4,
						attributes: {
							name: 'Chikpea.webp',
							alternativeText:
								'https://www.allrecipes.com/thmb/RDWRSUeFeQC16tto1abdk1OkNZk=/282x188/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/7557150-003b399503054e95873262027ed0619a.jpg',
							caption: null,
							width: 750,
							height: 750,
							formats: {
								thumbnail: {
									name: 'thumbnail_Chikpea.webp',
									hash: 'thumbnail_Chikpea_283d65d45c',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 156,
									height: 156,
									size: 6.06,
									url: '/uploads/thumbnail_Chikpea_283d65d45c.webp'
								},
								small: {
									name: 'small_Chikpea.webp',
									hash: 'small_Chikpea_283d65d45c',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 500,
									height: 500,
									size: 36.66,
									url: '/uploads/small_Chikpea_283d65d45c.webp'
								}
							},
							hash: 'Chikpea_283d65d45c',
							ext: '.webp',
							mime: 'image/webp',
							size: 81.84,
							url: '/uploads/Chikpea_283d65d45c.webp',
							previewUrl: null,
							provider: 'local',
							provider_metadata: null,
							createdAt: '2023-03-28T19:59:34.844Z',
							updatedAt: '2023-03-29T15:22:58.223Z'
						}
					}
				}
			}
		},
		{
			id: 5,
			attributes: {
				title: 'Lamb Souvlaki',
				time: '3 hrs 25 mins',
				level: 'medium',
				createdAt: '2023-03-28T20:11:46.875Z',
				updatedAt: '2023-03-28T21:23:57.132Z',
				publishedAt: '2023-03-28T21:23:57.128Z',
				ingredients: {
					1: '⅓ cup olive oil',
					2: '1 ½ tablespoons freshly squeezed lemon juice',
					3: '1 ½ tablespoons red wine vinegar',
					4: '1 ½ tablespoons chopped fresh oregano',
					5: '2 cloves garlic, minced',
					6: '½ teaspoon salt',
					7: '¼ teaspoon ground black pepper',
					8: '1 ½ pounds boneless leg of lamb, trimmed of all fat and cut into 1-inch cubes'
				},
				instructions: {
					1: 'Whisk olive oil, lemon juice, red wine vinegar, oregano, garlic, salt, and pepper together in a medium bowl. Add cubed lamb and stir until lamb is coated with marinade. Cover and refrigerate 3 hours, or overnight. ',
					2: 'Preheat an outdoor grill for medium-high heat and lightly oil the grate. ',
					3: 'Thread marinated lamb onto skewers, reserving any remaining marinade. Grill skewers until desired doneness, 10 to 12 minutes, basting with the reserved marinade and turning occasionally for even cooking. '
				},
				img: {
					data: {
						id: 5,
						attributes: {
							name: 'lamb.webp',
							alternativeText:
								'https://www.allrecipes.com/thmb/qEptt89NcMXIR76tS2h1HRrYSA0=/282x188/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/6718233_original-2000-efa2c6a5a2be477ba1d8cd823ba7512c.jpg',
							caption: null,
							width: 750,
							height: 750,
							formats: {
								thumbnail: {
									name: 'thumbnail_lamb.webp',
									hash: 'thumbnail_lamb_51e8763598',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 156,
									height: 156,
									size: 7.19,
									url: '/uploads/thumbnail_lamb_51e8763598.webp'
								},
								small: {
									name: 'small_lamb.webp',
									hash: 'small_lamb_51e8763598',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 500,
									height: 500,
									size: 42.23,
									url: '/uploads/small_lamb_51e8763598.webp'
								}
							},
							hash: 'lamb_51e8763598',
							ext: '.webp',
							mime: 'image/webp',
							size: 78.26,
							url: '/uploads/lamb_51e8763598.webp',
							previewUrl: null,
							provider: 'local',
							provider_metadata: null,
							createdAt: '2023-03-28T20:11:36.146Z',
							updatedAt: '2023-03-29T15:22:12.848Z'
						}
					}
				}
			}
		},
		{
			id: 6,
			attributes: {
				title: 'Hoisin-Glazed Salmon',
				time: '1 hrs 10 mins',
				level: 'medium',
				createdAt: '2023-03-28T20:20:43.720Z',
				updatedAt: '2023-03-28T21:23:43.461Z',
				publishedAt: '2023-03-28T21:23:43.456Z',
				ingredients: {
					1: '⅓ cup reduced-sodium soy sauce',
					2: '¼ cup hoisin sauce',
					3: '2 tablespoons olive oil',
					4: '2 tablespoons fresh lemon juice',
					5: '1 tablespoon chili garlic sauce',
					6: '1 tablespoon grated fresh ginger root',
					7: '1 clove garlic, pressed',
					8: '6 (6 ounce) skinless, boneless salmon fillets'
				},
				instructions: {
					1: 'Whisk together soy sauce, hoisin sauce, olive oil, lemon juice, chili sauce, ginger, and garlic in a 9x13-inch baking dish. Place salmon fillets into the marinade and turn to evenly coat. Cover the dish with plastic wrap and marinate in the refrigerator for 30 minutes. ',
					2: 'Preheat the oven to 350 degrees F (175 degrees C). ',
					3: 'Remove and discard the plastic wrap from the salmon. Use a spoon to scoop up the marinade that has collected in the bottom of the baking dish and drizzle it over salmon fillets.',
					4: 'Bake in the preheated oven until salmon flakes easily with a fork, about 30 minutes. '
				},
				img: {
					data: {
						id: 6,
						attributes: {
							name: 'hunan.webp',
							alternativeText:
								'https://www.allrecipes.com/thmb/-X6aJve1qy91X1Yf4cOmselN-ro=/282x188/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/691817-61cfaca239174199ab1630ca38a34355.jpg',
							caption: null,
							width: 250,
							height: 250,
							formats: {
								thumbnail: {
									name: 'thumbnail_hunan.webp',
									hash: 'thumbnail_hoisin_a676b60d6a',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 156,
									height: 156,
									size: 6.03,
									url: '/uploads/thumbnail_hoisin_a676b60d6a.webp'
								}
							},
							hash: 'hoisin_a676b60d6a',
							ext: '.webp',
							mime: 'image/webp',
							size: 13.41,
							url: '/uploads/hoisin_a676b60d6a.webp',
							previewUrl: null,
							provider: 'local',
							provider_metadata: null,
							createdAt: '2023-03-28T20:20:28.649Z',
							updatedAt: '2023-03-29T15:31:08.431Z'
						}
					}
				}
			}
		},
		{
			id: 7,
			attributes: {
				title: 'Hunan Kung Pao',
				time: '1 hrs 15 mins',
				level: 'advanced',
				createdAt: '2023-03-28T20:33:28.278Z',
				updatedAt: '2023-03-29T15:28:58.039Z',
				publishedAt: '2023-03-28T21:23:50.349Z',
				ingredients: {
					1: '4 skinless, boneless chicken breast halves',
					2: '20 peeled and deveined large shrimp ',
					3: '4 teaspoons soy sauce',
					4: '4 teaspoons rice wine',
					5: '4 cloves garlic, minced',
					6: '16 dried red chile peppers, cut in half',
					7: '2 teaspoons Sichuan peppercorns ',
					8: '¼ cup dark soy sauce',
					9: '1 cup salted peanuts',
					10: '4 green onions, cut into 3 inch lengths',
					11: '2 teaspoons sesame oil '
				},
				instructions: {
					1: 'Combine chicken and shrimp in a mixing bowl along with 4 teaspoons soy sauce, 4 teaspoons rice wine, and 2 teaspoons sesame oil. Sprinkle with cornstarch, and mix until evenly combined. Set aside to marinate for 25 minutes. ',
					2: 'Heat 1/2 of the vegetable oil in a wok over high heat. Stir in chicken mixture and cook until chicken has turned white on the outside and is still a bit pink in the center, about 5 minutes. Remove chicken mixture and wipe the wok clean. ',
					3: 'Heat remaining vegetable oil over high heat. Stir in garlic; cook for a few seconds until garlic begins to turn brown. Stir in dried chiles and Sichuan peppercorns; cook and stir for a few seconds until peppers begin to darken.',
					4: 'Add red and green bell peppers, dark soy sauce, 2 tablespoons rice wine, and sugar. Bring to a boil and stir in chicken mixture. Cook until peppers are nearly tender and chicken is no longer pink in the center, about 5 minutes more. Stir in peanuts and green onion until green onion becomes limp. Stir in a few drops of sesame oil to serve.'
				},
				img: {
					data: {
						id: 11,
						attributes: {
							name: 'hunan.webp',
							alternativeText: null,
							caption: null,
							width: 250,
							height: 250,
							formats: {
								thumbnail: {
									name: 'thumbnail_hunan.webp',
									hash: 'thumbnail_hunan_e96ba9342b',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 156,
									height: 156,
									size: 6.03,
									url: '/uploads/thumbnail_hunan_e96ba9342b.webp'
								}
							},
							hash: 'hunan_e96ba9342b',
							ext: '.webp',
							mime: 'image/webp',
							size: 13.41,
							url: '/uploads/hunan_e96ba9342b.webp',
							previewUrl: null,
							provider: 'local',
							provider_metadata: null,
							createdAt: '2023-03-29T15:28:35.069Z',
							updatedAt: '2023-03-29T15:28:35.069Z'
						}
					}
				}
			}
		},
		{
			id: 8,
			attributes: {
				title: 'Cucumber Sunomono',
				time: '1 hrs 15 mins',
				level: 'easy',
				createdAt: '2023-03-28T20:40:45.953Z',
				updatedAt: '2023-03-28T21:23:35.640Z',
				publishedAt: '2023-03-28T21:23:35.638Z',
				ingredients: {
					1: '2 large cucumbers, peeled',
					2: '⅓ cup rice vinegar',
					3: '4 teaspoons white sugar',
					4: '1 teaspoon salt',
					5: '1 ½ teaspoons minced fresh ginger root'
				},
				instructions: {
					1: 'Cut cucumbers in half lengthwise and scoop out any large seeds. Slice crosswise into very thin slices.',
					2: 'In a small bowl combine vinegar, sugar, salt and ginger. Mix well. Place cucumbers inside of the bowl, stir so that cucumbers are coated with the mixture. Refrigerate the bowl of cucumbers for at least 1 hour before serving.'
				},
				img: {
					data: {
						id: 8,
						attributes: {
							name: 'cucumber.webp',
							alternativeText:
								'https://www.allrecipes.com/thmb/ZAMlYmDTjCbIGOWC9opVtG43es0=/750x0/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/4523461-5ba0ee1b47244ad593372e984b104a4f.jpg',
							caption: null,
							width: 750,
							height: 750,
							formats: {
								thumbnail: {
									name: 'thumbnail_cucumber.webp',
									hash: 'thumbnail_cucumber_0e61095137',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 156,
									height: 156,
									size: 5.84,
									url: '/uploads/thumbnail_cucumber_0e61095137.webp'
								},
								small: {
									name: 'small_cucumber.webp',
									hash: 'small_cucumber_0e61095137',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 500,
									height: 500,
									size: 27.75,
									url: '/uploads/small_cucumber_0e61095137.webp'
								}
							},
							hash: 'cucumber_0e61095137',
							ext: '.webp',
							mime: 'image/webp',
							size: 48.74,
							url: '/uploads/cucumber_0e61095137.webp',
							previewUrl: null,
							provider: 'local',
							provider_metadata: null,
							createdAt: '2023-03-28T20:40:39.207Z',
							updatedAt: '2023-03-29T15:19:57.873Z'
						}
					}
				}
			}
		},
		{
			id: 9,
			attributes: {
				title: 'Cajun Shrimp',
				time: '10 mins',
				level: 'easy',
				createdAt: '2023-03-28T20:48:29.253Z',
				updatedAt: '2023-03-28T21:22:59.546Z',
				publishedAt: '2023-03-28T21:22:59.543Z',
				ingredients: {
					1: '1 teaspoon paprika',
					2: '¾ teaspoon dried thyme',
					3: '¾ teaspoon dried oregano',
					4: '¼ teaspoon garlic powder',
					5: '¼ teaspoon salt',
					6: '¼ teaspoon ground black pepper',
					7: '¼ teaspoon cayenne pepper, or more to taste',
					8: '1 ½ pounds large shrimp, peeled and deveined',
					9: '1 tablespoon vegetable oil'
				},
				instructions: {
					1: 'Combine paprika, thyme, oregano, garlic powder, salt, pepper, and cayenne pepper in a sealable plastic bag; shake to mix. Add shrimp and shake to coat.',
					2: 'Heat oil in a large non-stick skillet over medium high heat. Cook and stir shrimp in hot oil until they are bright pink on the outside and the meat is no longer transparent in the center, about 4 minutes.'
				},
				img: {
					data: {
						id: 9,
						attributes: {
							name: 'cajun.webp',
							alternativeText:
								'https://www.allrecipes.com/thmb/Hyp284Q7lSNEV602otuuxz5L6fM=/750x0/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/9445061-4e13b989730e4fb0a6c71058741f06c7.jpg',
							caption: null,
							width: 750,
							height: 501,
							formats: {
								thumbnail: {
									name: 'thumbnail_cajun.webp',
									hash: 'thumbnail_cajun_4a5909b676',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 234,
									height: 156,
									size: 9.19,
									url: '/uploads/thumbnail_cajun_4a5909b676.webp'
								},
								small: {
									name: 'small_cajun.webp',
									hash: 'small_cajun_4a5909b676',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 500,
									height: 334,
									size: 28.27,
									url: '/uploads/small_cajun_4a5909b676.webp'
								}
							},
							hash: 'cajun_4a5909b676',
							ext: '.webp',
							mime: 'image/webp',
							size: 55.12,
							url: '/uploads/cajun_4a5909b676.webp',
							previewUrl: null,
							provider: 'local',
							provider_metadata: null,
							createdAt: '2023-03-28T20:43:56.680Z',
							updatedAt: '2023-03-29T15:19:15.303Z'
						}
					}
				}
			}
		},
		{
			id: 10,
			attributes: {
				title: 'Schupfnudeln',
				time: '40 mins',
				level: 'medium',
				createdAt: '2023-03-28T20:59:07.487Z',
				updatedAt: '2023-03-28T21:24:25.838Z',
				publishedAt: '2023-03-28T21:24:25.834Z',
				ingredients: {
					1: '2 large starchy potatoes, such as russet or Yukon Gold',
					2: '2 large eggs',
					3: '1 ¾ cups flour, or as needed',
					4: 'salt and freshly ground black pepper to taste',
					5: '1 pinch ground nutmeg',
					6: '2 tablespoons butter'
				},
				instructions: {
					1: 'Place potatoes into a large pot and cover with salted water; bring to a boil. Reduce heat to medium-low and simmer until tender, 20 to 25 minutes. Drain.',
					2: 'Peel cooked potatoes and place into a large bowl. Mash with a potato masher. Add eggs and flour and mix until well combined. Season with salt, pepper, and nutmeg.',
					3: 'Use your hands to shape potato dough into oblong dumplings with two pointy ends. If the dough sticks to your hands, rinse them under cold water.',
					4: 'Bring water to the boil in a large pot over high heat, then reduce temperature to a simmer. Drop small batches of the dumplings into the just simmering water. Cook until they float on the top. Remove dumplings with a slotted spoon and submerge in a bowl with cold water. Remove with a slotted spoon, put them in a colander, and drain well.',
					5: 'Heat butter in a skillet over medium heat and saute Schupfnudeln until golden brown and crisp from all sides, 5 to 10 minutes. Drain on paper towel and keep warm until all Schupfnudeln are done.'
				},
				img: {
					data: {
						id: 10,
						attributes: {
							name: 'schupfnudeln.webp',
							alternativeText:
								'https://www.allrecipes.com/thmb/LH-xEB-2SPL3kzT-zy1gnRb6hl4=/750x0/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/9144816-4a124a9888ea409f897316da89c92f04.jpg',
							caption: null,
							width: 750,
							height: 563,
							formats: {
								thumbnail: {
									name: 'thumbnail_schupfnudeln.webp',
									hash: 'thumbnail_schupfnudeln_ca9232e438',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 208,
									height: 156,
									size: 7.96,
									url: '/uploads/thumbnail_schupfnudeln_ca9232e438.webp'
								},
								small: {
									name: 'small_schupfnudeln.webp',
									hash: 'small_schupfnudeln_ca9232e438',
									ext: '.webp',
									mime: 'image/webp',
									path: null,
									width: 500,
									height: 375,
									size: 25.5,
									url: '/uploads/small_schupfnudeln_ca9232e438.webp'
								}
							},
							hash: 'schupfnudeln_ca9232e438',
							ext: '.webp',
							mime: 'image/webp',
							size: 46.87,
							url: '/uploads/schupfnudeln_ca9232e438.webp',
							previewUrl: null,
							provider: 'local',
							provider_metadata: null,
							createdAt: '2023-03-28T20:58:53.599Z',
							updatedAt: '2023-03-29T15:16:51.408Z'
						}
					}
				}
			}
		}
	],
	meta: {
		pagination: {
			page: 1,
			pageSize: 25,
			pageCount: 1,
			total: 10
		}
	}
}

export default data
